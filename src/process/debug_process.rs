...
pub struct DebugProcessNode{
    #[serde(default = "DebugProcessNode::default_print", skip_serializing_if = "std::ops::Not::not")]
    pub print: bool,
}

 impl DebugProcessNode {
    fn default_print() -> bool { false }
 }
...
impl NodeType for DebugProcessNode {
    ...
    async fn process(
        ...
        if self.print {
            println!("**** DEBUG ****: Got message {:?}, context {:?}", input, context.get_all_state());
        }
...
mod debug_tests {
    ...
    async fn process_echoes_message_and_no_connections() {
        ...
        // call process
        let node = DebugProcessNode{print:true};
        let out = node.process(original.clone(), &mut ctx)
            .await
