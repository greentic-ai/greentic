...
pub async fn deploy_flow_file(
    ...
    if let Some(channels) = json["channels"].as_array() {
        let names = channels.iter().filter_map(|c| c["name"].as_str()).map(String::from).collect();
        missing("channels", &names)?;
    }

    if let Some(tools) = json["tools"].as_array() {
        let names = tools.iter().filter_map(|c| c["name"].as_str()).map(String::from).collect();
        missing("tools", &names)?;
    }
    if let Some(processes) = json["processes"].as_array() {
        let names = processes.iter().filter_map(|c| c["name"].as_str()).map(String::from).collect();
