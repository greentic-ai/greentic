...
use serde_json::json;
use teloxide::{
prelude::*, types::{InputFile, MediaKind, Message as TelegramMessage, ParseMode}, utils::command::BotCommands,
};
...
fn extract_content(bot: Bot, msg: &TelegramMessage) -> Option<MessageContent> {
    ...
    if let MessageKind::Common(common) = &msg.kind {
        match &common.media_kind {
            MediaKind::Text(t) => {
                 Some(MessageContent::Text{text:t.text.clone()})
            },

            MediaKind::Photo(ph) => {
                let photo = ph.photo.last()?;
                Some(MessageContent::Media{media:MediaMetadata {
            ...
            MediaKind::Video(video) => Some(MessageContent::Media{media:MediaMetadata {
            ...
            MediaKind::Voice(voice) => Some(MessageContent::Media{media:MediaMetadata {
            ...
            MediaKind::Audio(audio) => Some(MessageContent::Media{media:MediaMetadata {
                kind: MediaType::AUDIO,
                file: FileMetadata {
            ...
            MediaKind::Document(doc) => Some(MessageContent::File{file:FileMetadata {
            ...
            MediaKind::Sticker(sticker) => Some(MessageContent::Media{media:MediaMetadata {
                kind: MediaType::IMAGE,
                file: FileMetadata {
            ...
            MediaKind::VideoNote(video_note) => Some(MessageContent::Media{media:MediaMetadata {
                kind: MediaType::VIDEO,
                file: FileMetadata {
            ...
            MediaKind::Animation(anim) => Some(MessageContent::Media{media:MediaMetadata {
                kind: MediaType::VIDEO,
                file: FileMetadata {
            ...
            MediaKind::Contact(contact) => Some(MessageContent::Event{event:Event {
            ...
            MediaKind::Location(loc) => Some(MessageContent::Event{event:Event {
            ...
            MediaKind::Venue(venue) => Some(MessageContent::Event{event:Event {
...
impl PluginHandler for TelegramPlugin {
    fn name(&self) -> NameResult {
        NameResult{name:"telegram".to_string()}
    }
    ...
    async fn send_message(&mut self, msg: MessageOutParams) -> MessageOutResult  {
        ...
        if to_list.is_empty() {
            ...
            error!(error);
            return MessageOutResult{ success: false, error: Some(error.into()) };
        } else {
            ...
            match tokio::spawn(async move {
                ...
                Ok(Ok(result)) => result,
                Ok(Err(error)) => MessageOutResult{success:false, error: Some(error.to_string()) },
                Err(error) => MessageOutResult{ success: false, error: Some(error.to_string()) },
            }
        }
    }
...
pub async fn send_telegram_to_list(
    ...
    for rcpt in recipients {
        ...
        if contents.len() == 0 {
            return Ok(MessageOutResult{ success: false, error: Some("Content was empty".to_string()) });
        }
            ...
            let result = match content {
                ...
                MessageContent::File { file } => {
                    let url = Url::parse(&file.url)
                        .map_err(|e| anyhow!("bad file URL: {e}"))?;
                ...
                MessageContent::Media { media } => {
                    let url = Url::parse(&media.file.url)
                        .map_err(|e| anyhow!("bad media URL: {e}"))?;
...
mod tests {
    ...
    async fn test_state_transitions_async() {
        ...
        assert_eq!(p.state().await, StateResult{state:ChannelState::STOPPED});

        assert!(p.start(InitParams { 
        ...
        assert_eq!(p.state().await, StateResult{state:ChannelState::RUNNING});
    ...
    async fn test_send_without_content_errors_async() {
        ...
        assert!(!p.send_message(MessageOutParams { message: msg}).await.success);
    }

    #[tokio::test]
    async fn test_send_and_receive_roundtrip_async() {
        let mut p = TelegramPlugin::default();
        assert!(p.init(InitParams { 
            version: "123".to_string(), 
            config: vec![], 
        ...
        assert_eq!(got.message.content, vec![MessageContent::Text{text:"hello".into()}]);
        ...
        let outgoing = ChannelMessage {
            ...
            content: vec![MessageContent::Text{text:"reply".into()}],
            thread_id: None,
            reply_to_id: None,
            metadata: Default::default(),
        };
        ...
        assert!(!p.send_message(MessageOutParams { message: outgoing}).await.success);
    }
    ...
    async fn test_set_config_and_secrets_async() {
        ...
        assert_eq!(&entry, "bar");
        let entry = p.get_secret("TELEGRAM_TOKEN").expect("telegra token not set");
